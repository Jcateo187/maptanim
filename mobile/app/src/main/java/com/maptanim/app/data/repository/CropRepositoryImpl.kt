...
private fun CropDto.toDomain(): Crop = Crop(
    ...
    pRatio = npk_p,
    kRatio = npk_k,
    optimalPhMin = optimal_ph_min,
    optimalPhMax = optimal_ph_max,
    idealSoils = suitable_soils.mapNotNull { parseSoilType(it) },
    suitableSoils = suitable_soils.mapNotNull { parseSoilType(it) },
...
internal fun getCropsWithAssetMetadata(context: Context?): List<Crop> {
    ...
    return metaList.map { meta ->
        ...
        Crop(
            ...
            imageUrl = meta.primaryPhotoUrl.ifBlank { existingDefault?.imageUrl },
...
internal val defaultPhilippineCrops = listOf(
    ...
    Crop(
        ...
        companionPlants = listOf("Eggplant", "String Beans"),
        avoidPlants = emptyList(),
        commonPests = listOf("Fruit Fly", "Powdery Mildew"),
        harvestIndicators = "Firm ribbed fruit before turning yellow/orange",
        description = "High-value medicinal bitter gourd vining crop."
    ),
    Crop(
        id = "crop_kangkong",
        name = "Kangkong",
