...
users
  ...
  │     └── community_reports (post_id → community_posts.id)
  └── notifications (user_id → users.id)

crops (static reference — 15 canonical crops, admin managed)
dss_rules (static reference — 58 companion rules, admin managed)
```

> 💡 **Purged / Frontend-Only Architecture**:
> - `farm_tiles` (45×45 grid) is procedural and rendered purely in Jetpack Compose Canvas memory.
> - `farm_objects` (scenery like trees, rocks, fences) is rendered purely via background image scenery.
> - Obsolete tables (`planting_monitors`, `planting_harvests`, `tile_plantings`, `farm_tiles`, `crop_profiles`, `activities`) were officially dropped in **Migration 020 (`020_cleanup_redundant_schema.sql`)**.
...
## 🔹 Table: `crop_plots` (Direct-Planted Crop Plots)

> 💡 **Direct Soil Canvas Architecture**: MapTanim strictly features **Direct-to-Soil Canvas Planting**. Farmers drag crops directly onto the soil grid without building raised beds. In the database, the table `public.crop_plots` stores the direct-planted crop area's position ($pos\_x, pos\_y$), grid dimensions ($width\_m, height\_m$), crop variety, soil classification, planted date, and growth status.

```sql
CREATE TABLE public.crop_plots (
    id              UUID            PRIMARY KEY DEFAULT gen_random_uuid(),
    farm_id         UUID            NOT NULL REFERENCES public.farms(id) ON DELETE CASCADE,
    plot_label      VARCHAR(20)     NOT NULL,       -- e.g., "PLOT 1", "ZONE A"
...
## 🔹 Table: `harvest_records` (Yield Tracking)

Updated via Migration 020 with bidirectional sync feeding mobile harvest history and Admin yield analytics.
...
CREATE POLICY "farmers_own_harvest_records" ON public.harvest_records
    FOR ALL USING (
        EXISTS (
            SELECT 1 FROM public.crop_plots cp
            JOIN public.farms f ON f.id = cp.farm_id
            WHERE cp.id = harvest_records.plot_id AND f.farmer_id = auth.uid()
        )
    );
```
